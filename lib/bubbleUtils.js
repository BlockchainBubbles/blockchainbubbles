...
export class Bubble {
  constructor(coin, maxValue, sizeKey, changeKey, container, favorites, onCoinClick, index = 0) {
    this.coin      = coin
    ...
    this.cw = containerWidth
    this.ch = containerHeight
    const padding = this.radius + 5
    this.x = padding + Math.random() * (containerWidth  - padding * 2)
    this.y = padding + Math.random() * (containerHeight - padding * 2)
    this.vx = (Math.random() - 0.5) * 0.5
    this.vy = (Math.random() - 0.5) * 0.5

    this._createElement(change, favorites, onCoinClick)
    this.el.style.opacity = '0'
    setTimeout(() => { this.el.style.opacity = '1' }, index * 20)
  }
  ...
  _createElement(change, favorites, onCoinClick) {
    ...
    const showImage = this.radius > 20
    ...
    if (Math.abs(change) > 0.15) {
      ...
      el.innerHTML = `
        <div class="bubble-content pointer-events-none" style="text-shadow:0 1px 2px rgba(0,0,0,0.6);display:flex;flex-direction:column;align-items:center;justify-content:center;padding:5px;">
          ${showImage ? `<img src="${thumbSrc}" alt="${this.coin.name} icon"
