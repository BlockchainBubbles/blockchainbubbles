...
function getCoinImageUrl(imageUrl, radius) {
  if (!imageUrl) return imageUrl
  return radius >= 25
    ? imageUrl.replace('/large/', '/small/')
    : imageUrl.replace('/large/', '/thumb/')
}
...
export class Bubble {
  ...
  _createElement(change, favorites, onCoinClick) {
    ...
    // Inner content
    const imgSrc = `/api/image?url=${encodeURIComponent(getCoinImageUrl(this.coin.image, this.radius))}`

    if (Math.abs(change) > 0.15) {
      const imageSize      = this.radius * 0.5
      const imageSizeRound = Math.round(imageSize)
      ...
      el.innerHTML = `
        <div class="bubble-content pointer-events-none" style="text-shadow:0 1px 2px rgba(0,0,0,0.6);display:flex;flex-direction:column;align-items:center;justify-content:center;padding:5px;">
          <img src="${imgSrc}" alt="${this.coin.name} icon"
               width="${imageSizeRound}" height="${imageSizeRound}"
